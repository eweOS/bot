function split_command_once(args: string, split = ' ') {
	var first_elem = null;
...
function testarg_number(arg: string) {
	return /^\d+$/.test(arg);
}

function testarg_pkgname(arg: string) {
	if (!arg) return false;
	const regex = new RegExp('^[.0-9a-zA-Z_-]+$', 'g');
	return regex.test(arg);
}

function testarg_string_not_quoted(arg: string) {
